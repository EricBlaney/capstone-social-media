...
const {
  db,
  models: { User, Post, Like, Comment, Photo, Message },
} = require("../server/db");
...
async function seed() {
  ...
  const photos = await Promise.all([
    Photo.create({
      photoUrl: faker.image.cats(240, 240, true),
      userId: 1,
      postId: 1,
    }),
    Photo.create({
